const Map<String, String> hindiStrings = {
  'sangamner_ai': 'संगमनेर एआय',
  'home_title': 'संगमनेर की विश्वसनीय\nसेवाएँ खोजें',
  'home_subtitle': 'टॉप-रेटेड सैलून, क्लीनिक, रिपेयर सर्विस\nऔर भी बहुत कुछ — सब एक ही जगह।',
  'welcome_to_baap_company': 'बाप कंपनी में आपका स्वागत है',
  'change_language': 'भाषा बदलें',
  'marathi': 'मराठी',
  'english': 'अंग्रेजी',
  'hindi': 'हिंदी',
  'login': 'लॉगिन करें',
  'email_or_mobile': 'ईमेल या मोबाइल',
  'enter_email_or_mobile': 'ईमेल या मोबाइल दर्ज करें',
  'password': 'पासवर्ड',
  'enter_password': 'पासवर्ड दर्ज करें',
  'forgot_password': 'पासवर्ड भूल गए?',
  'dont_have_account': 'क्या आपके पास खाता नहीं है?',
  'sign_up': 'साइन अप करें',
  'logging_in': 'लॉगिन हो रहा है...',
  'by_signing_in': 'साइन इन करके, आप हमारी सहमत हैं',
  'terms_of_services': 'सेवा की शर्तें',
  'and': 'और',
  'privacy_policy': 'गोपनीयता नीति',
  'please_login_email_password': 'कृपया ईमेल और पासवर्ड से लॉगिन करें',
  'welcome_back': 'वापसी पर स्वागत है',
  'enter_credentials': 'स्टूडियो डैशबोर्ड तक पहुंचने के लिए अपनी साख दर्ज करें।',
  'mobile_number': 'मोबाइल नंबर',
  'use_otp_instead': 'इसके बजाय ओटीपी का उपयोग करें?',
  'remember_device': 'डिवाइस याद रखें',
  'enter_atelier': 'स्टूडियो में प्रवेश करें',
  'send_otp': 'ओटीपी भेजें',
  'new_to_suite': 'प्रबंधन सुइट में नए हैं? ',
  'join_as_partner': 'पाटनर म्हणून सामील व्हा',
  'bespoke_atelier': 'द बिस्पोक एटेलियर',
  'dashboard': 'डैशबोर्ड',
  'orderhistory': 'ऑर्डर इतिहास',

  // ── Home Screen ──
  'hello': 'नमस्ते',
  'active_orders_subtitle': 'आपकी वर्कशॉप में {count} सक्रिय ऑर्डर चल रहे हैं',
  'ai_insights': 'एआई इनसाइट्स',
  'ai_insights_delayed': 'सभी इनसाइट्स। आज आपके {count} ऑर्डर देरी से हैं।',
  'ai_insights_suggestion': 'ग्राहक संतुष्टि के लिए इन्हें प्राथमिकता दें, आगामी कपड़े के ऑर्डर कम करने का सुझाव है।',
  'view_delayed_orders': 'देरी वाले ऑर्डर देखें',
  'dismiss': 'खारिज करें',
  'create_order': 'ऑर्डर बनाएं',
  'customer_label': 'ग्राहक',
  'total_orders': 'कुल ऑर्डर',
  'pending_label': 'लंबित',
  'delivered': 'डिलीवर हुए',
  'collected': 'वसूली',
  'this_month': 'इस महीने',
  'recent_boutique_activity': 'हाल की बुटीक गतिविधि',
  'see_all': 'सभी देखें',
  'no_recent_activity': 'कोई हालिया गतिविधि नहीं',
  'financial_overview': 'वित्तीय अवलोकन',
  'monthly_target': '₹{amount} मासिक लक्ष्य में से',
  'achieved': '{percent}% प्राप्त',
  'remaining': '{percent}% शेष',
  'in_progress': 'प्रगति में',

  // ── Add Customer Screen ──
  'add_customer': 'ग्राहक जोड़ें',
  'save': 'सहेजें',
  'contact_details': 'संपर्क विवरण',
  'full_name': 'पूरा नाम',
  'full_name_hint': 'उदा. सेबस्टियन वेल',
  'please_enter_full_name': 'कृपया पूरा नाम दर्ज करें',
  'phone_number_label': 'फ़ोन नंबर',
  'enter_10_digit_number': '10 अंकों का नंबर दर्ज करें',
  'email_address_label': 'ईमेल पता',
  'email_hint': 'customer@domain.com',
  'save_customer': 'ग्राहक सहेजें',
  'customer_added_successfully': 'ग्राहक सफलतापूर्वक जोड़ा गया!',
  'error_prefix': 'त्रुटि',

  // ── Confirm Customer Bottom Sheet ──
  'review_client_profile': 'ग्राहक प्रोफ़ाइल की समीक्षा करें',
  'confirm_new_customer': 'नए ग्राहक की पुष्टि करें',
  'verify_profile_details': 'कृपया अंतिम रूप देने से पहले बिस्पोक प्रोफ़ाइल\nविवरण सत्यापित करें।',
  'contact': 'संपर्क',
  'full_name_label': 'पूरा नाम',
  'phone_label': 'फ़ोन',
  'email_label': 'ईमेल',
  'preferences': 'प्राथमिकताएँ',
  'priority': 'प्राथमिकता',
  'urgent': 'अत्यावश्यक',
  'standard': 'सामान्य',
  'fit_style': 'फ़िट शैली',
  'fabric_class': 'कपड़ा वर्ग',
  'super_150s_wool': 'सुपर 150s ऊन',
  'internal_notes': 'आंतरिक नोट्स',
  'edit_details': 'विवरण संपादित करें',
  'cancel_button': 'रद्द करें',
  'confirm_profile': 'प्रोफ़ाइल की पुष्टि करें',

  // ── Customers Screen ──
  'customers_title': 'ग्राहक',
  'manage_bespoke_clients': 'अपने {count} बिस्पोक ग्राहकों का प्रबंधन करें',
  'search_clients_hint': 'नाम या फ़ोन से ग्राहक खोजें...',
  'no_more_customers': 'और कोई ग्राहक नहीं',
  'bill_prefix': 'बिल',
  'status_label': 'स्थिति',
  'no_email_provided': 'ईमेल उपलब्ध नहीं',
  'view_profile': 'प्रोफ़ाइल देखें',
  'bill_no_label': 'बिल नं.',
  'generate_ai_style_profile': 'एआई स्टाइल प्रोफ़ाइल बनाएं',

  // ── Staff Management Screen ──
  'staff_management': 'कर्मचारी प्रबंधन',
  'staff_management_desc': 'अपने कार्यशाला के कुशल कारीगरों को व्यवस्थित और मॉनिटर करें।',
  'add_staff': 'कर्मचारी जोड़ें',
  'active_upper': 'सक्रिय',
  'total_artisans': 'कुल कारीगर',
  'current_workload': 'वर्तमान कार्यभार',
  'orders': 'ऑर्डर्स',
  'efficiency_rate': 'दक्षता दर',
  'staff_artisan_upper': 'कारीगर',
  'assigned_orders': 'सौंपे गए ऑर्डर्स',
  'over_capacity': 'क्षमता से अधिक',
  'capacity_label': 'क्षमता:',
  'onboard_new_artisan': 'नए कारीगर को जोड़ें',
  'retry': 'पुनः प्रयास करें',

  // ── Gallery Screen ──
  'gallery_curated_portfolio': 'क्यूरेटेड पोर्टफोलियो',
  'gallery_archive_of_excellence': 'उत्कृष्टता का\nसंग्रह',
  'gallery_add_portfolio_item': 'पोर्टफोलियो आइटम जोड़ें',
  'gallery_tap_to_upload_photo': 'फोटो अपलोड करने के लिए टैप करें',
  'gallery_item_title': 'आइटम का शीर्षक',
  'gallery_eg_title': 'उदा. मिडनाइट ब्लू थ्री-पीस',
  'gallery_material_subtitle': 'सामग्री / उपशीर्षक',
  'gallery_eg_subtitle': 'उदा. सुपर 150 मेरिनो वूल',
  'gallery_add_to_archive': 'संग्रह में जोड़ें',
  'gallery_filter_all_works': 'सभी कार्य',
  'gallery_filter_suits': 'सूट',
  'gallery_filter_casual': 'कैज़ुअल',
  'gallery_filter_fabrics': 'कपड़े',
  'gallery_filter_fittings': 'फिटिंग',

  // ── Staff Hello Screen ──
  'staff_portal_title': 'कर्मचारी\nपोर्टल',
  'queue_overview': 'कतार अवलोकन',
  'active_tasks_for': '{name} के लिए सक्रिय कार्य',
  'search_orders_hint': 'ऑर्डर खोजें...',
  'priority_label': 'प्राथमिकता',
  'total_tasks_label': 'कुल कार्य',
  'in_progress_upper': 'प्रगति पर',
  'high_priority_upper': 'उच्च प्राथमिकता',
  'completed_upper': 'पूरा हुआ',
  'no_tasks_in_queue': 'कतार में कोई कार्य नहीं है।',
  'tasks_tab': 'कार्य',
  'orders_tab': 'ऑर्डर्स',
  'log_out': 'लॉग आउट',
  'ai_efficiency_suggest': 'एआई दक्षता सुझाव',
  'batch_cutting_recommendation': 'ORD-0042 और ORD-0050 के लिए बैच कटिंग की सिफारिश की जाती है। दोनों में समान वजन वाली इंटरफ़ेसिंग का उपयोग किया जाता है।',
  'optimize_workflow': 'कार्यप्रवाह अनुकूलित करें',
  'fabric_availability': 'कपड़े की उपलब्धता',
  'navy_silk': 'नेवी सिल्क (120s)',
  'in_stock_upper': 'स्टॉक में',
  'tweed_harris': 'टवीड (हैरिस)',
  'low_stock_upper': 'कम स्टॉक',
  'client_label': 'क्लाइंट: ',
  'due_label': 'नियत: ',

  // ── Orders Management Screen ──
  'production_label': 'प्रोडक्शन',
  'orders_title': 'ऑर्डर',
  'search_customer_or_id': 'ग्राहक या आईडी खोजें...',
  'no_orders_found': 'कोई ऑर्डर नहीं मिला',
  'created_by': 'द्वारा बनाया गया:',
  'inv_prefix': 'INV:',
  'total_count_label': '{count} कुल',

  // ── Add Staff Screen ──
  'add_staff_title': 'कर्मचारी जोड़ें',
  'basic_information': 'बुनियादी जानकारी',
  'full_name_required': 'पूरा नाम *',
  'phone_number_required': 'फ़ोन नंबर *',
  'email_optional': 'ईमेल (वैकल्पिक)',
  'designation_required': 'पदनाम *',
  'eg_john_doe': 'उदा. जॉन डो',
  'eg_senior_stylist': 'उदा. सीनियर स्टाइलिस्ट',
  'phone_must_be_10_digits': 'फ़ोन नंबर ठीक 10 अंकों का होना चाहिए',
  'please_enter_valid_email': 'कृपया एक मान्य ईमेल पता दर्ज करें',
  'please_fill_required_fields': 'कृपया सभी आवश्यक फ़ील्ड भरें',
  'staff_invited_successfully': 'कर्मचारी को सफलतापूर्वक आमंत्रित किया गया',
  'invite_staff': 'कर्मचारी आमंत्रित करें',

  // ── Shell Navigation & Drawer ──
  'drawer_order_history': 'ऑर्डर इतिहास',
  'drawer_customers': 'ग्राहक',
  'drawer_staff': 'कर्मचारी',
  'drawer_gallery': 'गैलरी',
  'drawer_analytics': 'एनालिटिक्स',
  'drawer_language': 'भाषा',
  'drawer_logout': 'लॉग आउट',
  'drawer_loading': 'लोड हो रहा है...',
  'drawer_premium_tier': 'प्रीमियम टियर',
  
  'nav_home': 'होम',
  'nav_orders': 'ऑर्डर्स',
  'nav_customers': 'ग्राहक',
  'nav_staff': 'कर्मचारी',
  'nav_gallery': 'गैलरी',

  // ── Select Customer Screen ──
  'select_customer_title': 'ग्राहक चुनें',
  'step_1_order_flow': 'चरण 1 — ऑर्डर प्रवाह',
  'who_is_this_order_for': 'यह ऑर्डर किसके लिए है?',
  'choose_existing_customer_desc': 'उनका बिस्पोक ऑर्डर बनाना शुरू करने के लिए\nएक मौजूदा ग्राहक चुनें।',
  'search_by_name_or_phone': 'नाम या फ़ोन से खोजें...',
  'add_new_customer': 'नया ग्राहक जोड़ें',
  'create_profile_first_time': 'पहली बार आए क्लाइंट के लिए प्रोफ़ाइल बनाएं',
  'all_customers': 'सभी ग्राहक',
  'selected_for_this_order': 'इस ऑर्डर के लिए चुना गया',
  'change': 'बदलें',
  'proceed_to_order': 'ऑर्डर के लिए आगे बढ़ें',
  'no_customers_found': 'कोई ग्राहक नहीं मिला',

  // ── Staff Order Screen ──
  'production_text': 'उत्पादन',
  'stage_text': 'चरण',
  'in_progress_caps': 'प्रगति पर',
  'confirm_order': 'ऑर्डर की पुष्टि करें',
  'step_measurement': 'माप',
  'step_fabric': 'कपड़ा',
  'step_cutting': 'कटिंग',
  'step_stitching': 'सिलाई',
  'step_fitting': 'फिटिंग',
  'step_ready': 'तैयार',
  'workshop_comments': 'वर्कशॉप टिप्पणियाँ',
  'workshop_comments_hint': 'प्रगति, कपड़े की बारीकियों, या सिलाई टीम के लिए समायोजन का वर्णन करें...',
  'chip_fabric_arrived': 'कपड़ा आ गया',
  'chip_pattern_completed': 'पैटर्न पूरा हुआ',
  'chip_minor_delay': 'मामूली देरी: धागा आपूर्ति',
  'update_notify_manager': 'अपडेट करें और मैनेजर को सूचित करें',
  'notifications_sent_notice': 'लीड मैनेजर और क्लाइंट को रॉयल स्टिच ऐप के माध्यम से नोटिफिकेशन भेजे जाएंगे।',
  'premium_member_gold': 'प्रीमियम मेंबर • गोल्ड क्लास',
  'deadline_label': 'अंतिम तिथि',
  'fabric_label': 'कपड़ा',
  'lining_label': 'अस्तर',
  'view_full_measurement': 'पूर्ण माप प्रोफ़ाइल देखें',
  'ai_workshop_suggest': 'AI वर्कशॉप सुझाव',
  'ai_suggest_desc': 'कपड़े के वजन (320g) के आधार पर, हम सुझाव देते हैं कि पहली सिलाई से पहले ड्रेप स्थिरता सुनिश्चित करने के लिए कटिंग के बाद 48 घंटे का हैंग-टाइम हो।',
  'accept_adjustment': 'समायोजन स्वीकार करें',
  'staff_order_history': 'ऑर्डर इतिहास',
  'staff_nav_tasks': 'कार्य',
  'staff_nav_orders': 'ऑर्डर्स',
  'staff_nav_inventory': 'इन्वेंटरी',
  'staff_nav_chat': 'चैट',
};
